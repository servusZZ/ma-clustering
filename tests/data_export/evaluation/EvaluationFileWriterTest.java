...
class EvaluationFileWriterTest {
	...
	void testWriteAndAppendToEvaluationFile() throws IOException {
		...
		ProjectEvaluationEntry projectMetrics = new ProjectEvaluationEntry(1, "biojavaTest", 5, 38, 120, 50, 20);
		EvaluationEntry evaluationEntry = new EvaluationEntry("HAC", 3, 25, 3, 3, projectMetrics, clusterMetrics);
		writer.writeEvaluationEntry(evaluationEntry);
		writer.writeEvaluationEntry(evaluationEntry);
		evaluationEntry = new EvaluationEntry("RANDOM", 2, 10, 3, 3, projectMetrics, null);
		writer.writeEvaluationEntry(evaluationEntry);
	}
	@Test
	void testWriteNewFileWithNullClusterMetrics() throws IOException {
		...
		ProjectEvaluationEntry projectMetrics = new ProjectEvaluationEntry(15, "biojavaTest", 10, 90, 20, 50, 20);
		EvaluationEntry evaluationEntry = new EvaluationEntry("RANDOM", 3, 25, 3, 3, projectMetrics, null);
		...
		evaluationEntry = new EvaluationEntry("HAC", 3, 25, 3, 5, projectMetrics, clusterMetrics);
